...
Article pages pull title and thumbnail from markdown frontmatter and auto-generate a 160-character description from the body. Podcast episode pages are generated by fetching the Buzzsprout RSS feed at build time.
...
The same file contains the SPA catch-all rewrite (`/(.*) → /index.html`) that enables client-side routing in production.
